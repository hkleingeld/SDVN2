...
// 		}
// 		uart_write('\n');
		//uint16_t min = samples[36];
		//Extract features
 		uint16_t max = Maximum(NR_OF_SAMPLES, &samples[0]);
 //		uint16_t min = Average(4, &samples[34]);
		uart_write_string(itoa(max,itoabuffer,10));
// 		uart_write(',');
// 		uart_write_string(itoa(min,itoabuffer,10));
 		uart_write('\n');
// 		
//  		if(ADC_select == 3){
// 			uart_write(',');
...
// 			
// 			AVG_SAMPLES = 30; //pow((deviation * (SIGMA * 10) / StdDev_GetMean(lightMaxStdDev)),2) + 1;
// 					
// 			if(AVG_SAMPLES > 99){
// 				AVG_SAMPLES = 99;
// 			}
...
// 
// 				//if avg > mean+4*std/correction factor -> Detection
// 				float Threshold = (SIGMA * deviation/10)/sqrt(AVG_SAMPLES);
// 				if (Threshold < 2) Threshold = 2;
// 				
// 				if((Avg > StdDev_GetMean(lightMaxStdDev) + Threshold) ||
// 				   (Avg < StdDev_GetMean(lightMaxStdDev) - Threshold)
// 				)
// 				{
...
// // 				uart_write_string(" +- ");
// // 				uart_write_string(itoa(Threshold,itoabuffer,10));
// // 				uart_write_string("  n = ");
...
// 				
// 				if(miscCounter % (SIZE_INPUTBUFFER + 50) == 0) lastDeviation++; //why 150? 200 too much of a wait while testing a lot of data points. Other than that, no clue
